...
function closeWelcomeModal() {
  ...
  localStorage.setItem('alerta_welcomed', 'true');
}
...
if (!localStorage.getItem('alerta_welcomed')) {
...
function plotFires(data) {
  ...
  data.forEach(f => {
    ...
    m.bindPopup(`
        <div class="pt"><i class="fa-solid fa-fire" style="color:#FF6B35"></i> Foco Detectado</div>
        <div class="pm">
          <span class="pfrp">Potencia (FRP):</span> ${frp.toFixed(1)} MW<br>
          <span>Confianza:</span> ${conf}<br>
          <span>Coordenadas:</span> ${lat.toFixed(4)}, ${lon.toFixed(4)}
        </div>
        <button class="dispatch-btn" onclick="dispatchAlert(${lat}, ${lon}, ${frp}, '${conf}', '${acq}', '${activeSensor}', '${reg}', 'urgente', this)">
          <i class="fa-solid fa-satellite-dish"></i> ENVIAR AL GUARDIA (Urgente)
        </button>
        <button class="dispatch-btn-review" onclick="dispatchAlert(${lat}, ${lon}, ${frp}, '${conf}', '${acq}', '${activeSensor}', '${reg}', 'revision', this)">
          <i class="fa-solid fa-clipboard-check"></i> Asignar para revisión
        </button>
      `);
    markers.push(m);
  });
...
function toggleGuardMode() {
  ...
  if (guardMode) {
    ...
    btn.innerHTML = '<i class="fa-solid fa-person-military-pointing"></i> Modo Guardia ACTIVO';
    map.getContainer().style.cursor = 'crosshair';
    showToast('Haz clic en el mapa para colocar un punto de vigilancia', 'warn');
  } else {
    btn.classList.remove('on');
    btn.innerHTML = '<i class="fa-solid fa-location-crosshairs"></i> Colocar Guardia';
...
map.on('click', async function(e) {
  ...
  m.bindPopup(`
    <div class="guard-popup-name"><i class="fa-solid fa-shield-halved"></i> ${name}</div>
    <div class="guard-popup-meta">
      ...
      <span style="color:#F7B731;"><i class="fa-solid fa-user"></i> Usuario: <b>${username}</b></span><br>
      <span style="color:#F7B731;"><i class="fa-solid fa-key"></i> Clave: <b>${password}</b></span>
    </div>
  `);
  guardMarkers.push(m);
  ...
  showToast(`Guardia ${name} registrado`, 'success');
});
...
function showToast(msg, type = 'success') {
  ...
  const icon = type === 'warn' ? '<i class="fa-solid fa-triangle-exclamation" style="color:#FF6B35"></i>' : '<i class="fa-solid fa-circle-check" style="color:#2ED573"></i>';
